...
async function main() {
    ...
    if(isOk) {
        ...
        let articles = await getList();
        
        // $.log(article);
        await like(articles[0].targetId);
        await wait(2,5);
        await unlike(articles[0].targetId);
        

        await like(articles[1].targetId);
        await wait(2,5);
        await unlike(articles[1].targetId);

        
        await forward(articles[0].targetId);
        await wait(1,2);
        await updateState();

        await read(articles[0].targetId);
    }       
    // isOk && (await getList())
...
async function read(targetId) {
     return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://mse.digitalvolvo.com/mweb/api/app/mweb/mms/api/v1/triggerTaskEvent`;
        body = `{"postId": "${targetId}", "eventCode": "C_EVENT_000039"}`//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/mweb/api/app/mweb/mms/api/v1/triggerTaskEvent/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
...
async function updateState() {
    return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://apigateway.digitalvolvo.com/app/membership/api/v1/updateTaskAwardState`;
        body = '{"channel": 1, "taskRecord": [272194599]}'//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/app/membership/api/v1/updateTaskAwardState/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
...
async function forward(targetId) {
    return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://apigateway.digitalvolvo.com/app/community/api/v1/post/forward/${targetId}`;
        body = '{}'//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/app/community/api/v1/post/forward/${targetId}/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
...
async function like(targetId) {
    return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://apigateway.digitalvolvo.com/app/community/api/v3/posts/${targetId}/like`;
        body = '{}'//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/app/community/api/v3/posts/${targetId}/like/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
...
async function unlike(targetId) {
    return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://apigateway.digitalvolvo.com/app/community/api/v3/posts/${targetId}/unlike`;
        body = '{}'//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/app/community/api/v3/posts/${targetId}/unlike/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
...
async function getList() {
    return new Promise((resolve) => {
        signTime = get_Time();
        url = `https://apigateway.digitalvolvo.com/app/community/api/v2/recommendation/list`;
        body = '{"pageSize": 20, "pageIndex": 1}'//`{"memberId":"${$.memberId}"}`
        signature = CryptoJS.HmacSHA256(`SDK-HMAC-SHA256\n${signTime}\n${CryptoJS.SHA256(`POST\n/app/community/api/v2/recommendation/list/\n\nauthorization:Bearer ${$.accessToken}\nhost:apigateway.digitalvolvo.com\nx-sdk-content-sha256:UNSIGNED-PAYLOAD\nx-sdk-date:${signTime}\nx-token:${$.jwtToken}\n\nauthorization;host;x-sdk-content-sha256;x-sdk-date;x-token\nUNSIGNED-PAYLOAD`).toString()}`, $.key).toString();
        headers = {
            ...gethead(signTime, 'authorization;host;x-sdk-content-sha256;x-sdk-date;x-token', signature),
            "X-Token": $.jwtToken,
            "Authorization": `Bearer ${$.accessToken}`
        ...
        $.post(rest, (err, resp, data) => {
            try {
                ...
                if (obj?.code == 200) {
                    ...
                    // $.log(JSON.stringify(obj.data.items[0]));
                    articles = obj.data.items;
                }
            } catch (e) {
                $.logErr(e, "❌请重新登陆更新Token");
            } finally {
